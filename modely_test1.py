...
class BertCoAttention(nn.Module):
    ...
    # 多头自注意力机制中对输入张量进行形状调整，以便在不同的头之间进行并行计算 将输入张量的最后一个维度（通常是隐藏层维度）转换为 [num_attention_heads, attention_head_size] 的形式，并通过 permute 调整维度顺序。
...
class DMSD(nn.Module):
    ...
    def calculate_conflict(self, v_s, v_l, W_con):
        # 连接语义表示、潜在表示以及它们的差的绝对值
        concat_features = torch.cat([v_s, v_l, torch.abs(v_s - v_l)], dim=-1)
        # 通过线性层进行计算
        return W_con(concat_features)

    def forward(self,input_mask,bert_mask_add, bert_indices_add, img_trans,glm_mask_add, glm_indices_add,image_path,mode="train"):
        #data preparation
        ...
        mean, visual = self.vit(img_trans)      # viasual(batchsize,197,768)

        visual_latent = self.scsa(img_trans)
        mean2,visual_latent = self.vit(visual_latent)
        ...
        vt,attention_probs_vt=self.text_image_attention(visual,text_output,text_extended_mask) #(batchsize,197,768)
        tv,attention_probs_vt=self.text_image_attention(text_output,visual,text_extended_mask) #(batchsize,100,768)
        vs=self.norm1(vt)[:, 0]
        ts=self.norm2(tv)[:, 0]
        ...
        vl=visual_latent #(batchsize,197,768)
        cl,attention_probs_cl=self.text_image_attention(vl,tl,text_extended_mask)
        attention_map = attention_probs_cl.mean(dim=1).detach().cpu().numpy()  # 平均多个注意力头
        tl=self.norm2(tl)[:, 0]
        vl=self.norm1(vl)[:, 0]
        ...
        h1=cs*(torch.div(norml.unsqueeze(1), norms.unsqueeze(1)))
        ...
        if mode=="train":
            output = y_pred #(48,2)
            return output,scale,polar_vector
        else:
            output = y_pred #(48,2)
            return output,scale,polar_vector,attention_map,vs,ts,cs,tl,vl,cl
